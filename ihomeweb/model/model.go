...
//可以直接继承gorm的预定义字段,ID,创建时间,更新时间,删除时间
//注意:如果有删除时间字段,删除的时候不会真的删除,只会添加删除时间
...
func InitDb(){
	...
	db.AutoMigrate(new(User),new(House),new(HouseImage),new(Area),new(OrderHouse),new(Facility))
